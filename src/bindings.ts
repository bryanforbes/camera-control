...
export const commands = {
  async ready(): Promise<null> {
    return await TAURI_INVOKE('ready');
  },
  ...
  async goToPreset(preset: number): Promise<null> {
    return await TAURI_INVOKE('go_to_preset', { preset });
  },
  async setPreset(preset: number): Promise<null> {
    return await TAURI_INVOKE('set_preset', { preset });
  },
...
export const events = __makeEvents__<{
  ...
  portStateEvent: 'port-state-event',
});
...
export type Error =
  ...
  | { type: 'SerialPort' }
  | { type: 'PelcoD' };
export type PortStateEvent = { port: string | null };
